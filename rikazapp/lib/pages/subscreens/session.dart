...
// ------------------------------------------------------------
// SessionPage (Modified for Flexibility + Hardware Integration)
// ------------------------------------------------------------
...
print(
'✅ Session Started in DB with ID: $_currentSessionId, Planned Duration: $finalPlannedDuration min');
}
} catch (e) {
...
print(
'✅ Session ID: $_currentSessionId Ended successfully. Focus Time: $actualFocusDurationMinutes min, Progress: $progressLevel');
} catch (e) {
...
// Get WLED connection status from arguments
_wledConnected = widget.wledConnected ?? false;
...
// MODIFIED: Only start light AFTER a delay to ensure this is a real session start
// Not just opening the app
if (_wledConnected && !_lightInitialized) {
Future.delayed(const Duration(milliseconds: 500), () {
if (mounted && status == 'running') {
_wledService.startFocusLight();
_lightInitialized = true;
print('🔵 WLED: Session started - Focus light ON');
}
});
} else if (!_wledConnected) {
print('⚠️ WLED: Tools not connected, light control disabled');
}
...
// TURN OFF WLED LIGHT
if (_wledConnected && _lightInitialized) {
_wledService.stopLight();
print('⚫ WLED: Custom session ended - Light OFF');
}
...
// CHANGE TO BREAK LIGHT
if (_wledConnected && _lightInitialized) {
_wledService.startBreakLight();
print('🟡 WLED: Break started - Break light ON');
}
} else {
...
if (_wledConnected && _lightInitialized) {
_wledService.stopLight();
print('⚫ WLED: Pomodoro complete - Light OFF');
}
...
// RETURN TO FOCUS LIGHT
if (_wledConnected && _lightInitialized) {
_wledService.startFocusLight();
print('🔵 WLED: Focus resumed - Focus light ON');
}
}
}
}
...
if (_wledConnected && _lightInitialized) {
_wledService.stopLight();
print('⚫ WLED: Session quit - Light OFF');
}
...
// TURN OFF WLED LIGHT (only if it was initialized)
if (_wledConnected && _lightInitialized) {
_wledService.stopLight();
print('⚫ WLED: Session disposed - Light OFF');
}
...
decoration: BoxDecoration(
color:
(isBreak ? Colors.orange : Colors.blue).withOpacity(0.2),
